...
function PlaceholderSvg({ className }) {
  return (
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 360" className={className}>
...
// --- Dedicated Row Component (The Correct Approach) ---

const NftTransactionRow = memo(({ tx, chainId, getExplorerBaseUrl, shortenAddress }) => {
  const [metadata, setMetadata] = useState({
    name: `${tx.asset} #${parseInt(tx.tokenId, 16)}`,
    image: null,
  });
  ...
  useEffect(() => {
    ...
    fetchMetadata();
  }, [tx.rawContract.address, tx.tokenId, chainId, tx.asset]);
    ...
    <div className="text-sm text-gray-200 dark:text-gray-100 border-b border-gray-200 dark:border-gray-100 pb-2">
      <div className="flex items-center gap-3 w-full">
        <div className="flex-shrink-0">
          {isLoading ? (
            <div className="w-10 h-10 rounded object-cover bg-gray-200 dark:bg-gray-700 animate-pulse" />
          ) : metadata.image ? (
            <img 
              src={metadata.image} 
              alt={metadata.name}
        ...
        <div className="flex-grow min-w-0">
          ...
          <div className="flex justify-between items-center text-sm mt-1">
            <div className="text-black dark:text-white font-medium truncate" title={metadata.name}>
              {metadata.name}
            </div>
      ...
      <div className="flex items-center justify-between text-black dark:text-white mt-1">
        ...
        <button onClick={() => setIsExpanded(p => !p)} className="text-xs underline">
