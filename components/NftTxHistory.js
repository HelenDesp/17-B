...
export default function NftTxHistory({ address, chainId }) {
  ...
  const metadataCache = useRef(new Map()); // MODIFIED: Renamed from imageUrlCache for clarity
  const [nftNames, setNftNames] = useState({}); // MODIFIED: Added state to hold correct names
	...
	<div className="flex justify-between items-center mb-2">
      ...
      <div className="space-y-2">
        {paginated.length === 0 ? (
          ...
          paginated.map((tx, i) => {
            // MODIFIED: Use a stable, unique key to store and retrieve the name
            const uniqueKey = `${chainId}-${tx.rawContract.address}-${tx.tokenId}`;
            const displayName = nftNames[uniqueKey] || tx.asset;
              ...
              <div className="flex items-center gap-3 w-full">
                ...
                <div className="flex-shrink-0">
                  <NftImage 
                    ...
                    chainId={chainId}
                    onNameLoaded={(name) => {
                      setNftNames(prev => ({ ...prev, [uniqueKey]: name }));
                    }}
                  />
                </div>
