...
// Helper component to fetch and cache each unique NFT image
function NftImage({ contractAddress, tokenId, chainId, cache }) {
  const [imageUrl, setImageUrl] = useState('https://placehold.co/40');
  const [isLoading, setIsLoading] = useState(true);
  
  const getNftApiBaseUrl = (cId) => {
    const mapping = {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      ...
      const nftApiBaseUrl = getNftApiBaseUrl(chainId);
      if (!nftApiBaseUrl) {
          setIsLoading(false);
          return; 
      }
      ...
      try {
        const apiKey = "oQKmm0fzZOpDJLTI64W685aWf8j1LvDr"; // Using the same key for consistency
        const decimalTokenId = parseInt(tokenId, 16);
        const res = await fetch(
          `${nftApiBaseUrl}/nft/v3/${apiKey}/getNFTMetadata?contractAddress=${contractAddress}&tokenId=${decimalTokenId}`,
        ...
        if (typeof rawUrl === 'string' && rawUrl.trim() !== '') {
          ...
          if (trimmedUrl.startsWith("ipfs://")) {
            finalImageUrl = trimmedUrl.replace("ipfs://", "https://cloudflare-ipfs.com/ipfs/");
    ...
    fetchMetadata();
  }, [contractAddress, tokenId, chainId, cache]);
...
export default function NftTxHistory({ address, chainId, isConnected }) {
  ...
  return (
    <div className="p-4 bg-white border-b2 dark:bg-dark-200 shadow">
        ...
        <div className="space-y-2">
            ...
            paginated.map((tx, i) => (
                <div key={`${tx.hash}-${tx.tokenId}-${i}`} className="text-sm text-gray-200 dark:text-gray-100 border-b border-gray-200 dark:border-gray-100 pb-2">
        ...
        {txs.length > perPage && (
            <div className="flex justify-between items-center mt-4 text-sm">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setPage((p) => Math.min(Math.ceil(txs.length / 4), p + 1))} disabled={page * 4 >= txs.length} className="px-2 py-1 border rounded disabled:opacity-50">&gt;</button>
                    <button onClick={() => setPage(Math.ceil(txs.length/perPage))} disabled={page === Math.ceil(txs.length / perPage)} className="px-2 py-1 border rounded disabled:opacity-50">Last</button>
                </div>
            </div>
        )}
    </div>
