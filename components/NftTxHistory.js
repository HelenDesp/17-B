...
function getMoralisNftImageUrl(tx) {
  // Moralis metadata is often a stringified JSON, so we need to parse it first.
...
export default function NftTxHistory({ address, chainId }) {
  ...
  // Helper to convert wagmi chain ID to Moralis chain hex value
    ...
    const fetchMoralisTxs = async () => {
      ...
      try {
        ...
        const processedTxs = (data.result || []).map(tx => {
            ...
            // Return a unified object, preserving the original tx data
	...
	<div className="flex justify-between items-center mb-2">
      ...
      <div className="space-y-2">
        {paginated.length === 0 ? (
          ...
          paginated.map((tx, i) => (
            <div key={`${tx.transaction_hash}-${tx.token_id}-${i}`} className="text-sm text-gray-200 dark:text-gray-100 border-b border-gray-200 dark:border-gray-100 pb-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <img 
                    src={getMoralisNftImageUrl(tx)} 
                    alt={tx.name || 'NFT Image'}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex justify-between text-sm mt-1">
                      <div className="text-black dark:text-white font-medium truncate pr-2" title={`${tx.name} #${tx.token_id}`}>
                        {tx.name || tx.token_address} #{tx.token_id}
                      </div>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col items-end pl-2">
                    ...
                    <div className="text-black dark:text-white text-xs whitespace-nowrap mt-4">{formatShortDate(tx.block_timestamp)}</div>
                </div>
              </div>
              ...
              {expandedIndexes[i] && (
                <div className="mt-2 pl-12 space-y-1 text-xs">
                  <div className="flex items-center justify-between text-black dark:text-white">
                    <a href={`${getExplorerBaseUrl(chainId)}/tx/${tx.transaction_hash}`} target="_blank" rel="noopener noreferrer" className="underline">View on Explorer</a>
                  ...
                  <div className="text-black dark:text-white"><strong>From:</strong> <AddressDisplay address={tx.from_address} chainId={chainId} getExplorerBaseUrl={getExplorerBaseUrl} shortenAddress={shortenAddress} /></div>
                  <div className="text-black dark:text-white"><strong>To:</strong> <AddressDisplay address={tx.to_address} chainId={chainId} getExplorerBaseUrl={getExplorerBaseUrl} shortenAddress={shortenAddress} /></div>
