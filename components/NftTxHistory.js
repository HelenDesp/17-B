...
// The single contract address for your collection
const REVERSE_GENESIS_CONTRACT = "0x28D744dAb5804eF913dF1BF361E06Ef87eE7FA47";

// The Alchemy URL for Base chain
...
// This is a simple helper to get the image from the transaction data, if it exists.
function getImageUrlFromTx(tx) {
  ...
  return 'https://placehold.co/40'; // Fallback if no image is in the response
}
...
export default function NftTxHistory({ address, chainId }) {
  ...
  const getChainLabel = (cId) => "Base"; // Hardcoded for this component
  const getExplorerBaseUrl = (cId) => "https://basescan.org"; // Hardcoded for this component

  useEffect(() => {
    // Only run if a user is connected and is on the Base network (chainId 8453)
    if (!address || chainId !== 8453) {
        setTxs([]); // Clear transactions if not on the right chain
        return;
    };
        ...
        const fetchBody = (addrField, addrValue) => ({
          ...
          body: JSON.stringify({
            ...
            params: [{
              fromBlock: "0x0",
              [addrField]: addrValue,
              contractAddresses: [REVERSE_GENESIS_CONTRACT], // Filter by your specific contract
              category: ["erc721", "erc1155"],
              withMetadata: true, // Ask for the image and metadata
  ...
  return (
	...
	<div className="flex justify-between items-center mb-2">
      ...
      <div className="space-y-2">
        {paginated.length === 0 ? (
          <p className="text-gray-500 dark:text-white text-sm">No recent transactions for this collection.</p>
        ) : (
          paginated.map((tx, i) => (
