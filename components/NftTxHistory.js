...
// MODIFIED: NftImage now accepts chainId and onNameLoaded to fix bugs
function NftImage({ contractAddress, tokenId, cache, chainId, onNameLoaded = () => {} }) {
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      ...
      const cacheKey = `${chainId}-${contractAddress}-${tokenId}`; // Chain-specific cache key
...
export default function NftTxHistory({ address, chainId }) {
  ...
  const metadataCache = useRef(new Map()); // MODIFIED: Renamed from imageUrlCache
  const [nftNames, setNftNames] = useState({}); // MODIFIED: Added state for names
  ...
  useEffect(() => {
    ...
    setPage(1);
    setNftNames({}); // MODIFIED: Reset names on new fetch
	...
	<div className="flex justify-between items-center mb-2">
      ...
      <div className="space-y-2">
        {paginated.length === 0 ? (
          ...
          paginated.map((tx, i) => {
            // MODIFIED: Use the fetched name from state, with a fallback to the original tx.asset
                ...
                <div className="flex items-center gap-3 w-full">
                  <div className="flex-shrink-0">
                    <NftImage 
                      ...
                      tokenId={tx.tokenId} 
                      cache={metadataCache}
                      chainId={chainId} // Pass chainId prop
                      onNameLoaded={(name) => { // Pass callback prop
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="flex justify-between items-center text-sm mt-1">
                      {/* MODIFIED: Display name from state */}
