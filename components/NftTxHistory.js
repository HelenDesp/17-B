...
// MODIFIED: NftImage now accepts chainId and onNameLoaded props to fix the bugs
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      ...
      if (cache.current.has(cacheKey)) {
        ...
        // Pass cached name to parent if it exists
        if (cachedData.name) {
      ...
      try {
        // Using the API key from your original file
        const apiKey = "SLNvhiAdM71qDKy7veyeWn9ZD8XO11oY";
        const decimalTokenId = parseInt(tokenId, 16);
        // Fetch URL is now dynamic using the correct chainId
        ...
        // Extract full collection name and pass it to parent via callback
        const collectionName = data.collection?.name || data.contract?.name;
        if (collectionName) {
            onNameLoaded(collectionName);
        }
        
        // Cache both image and name together
...
export default function NftTxHistory({ address, chainId }) {
  ...
  const metadataCache = useRef(new Map()); // Renamed for clarity
  const [nftNames, setNftNames] = useState({}); // Added state to hold correct names
  ...
  useEffect(() => {
    ...
    setPage(1);
    setNftNames({}); // Reset names when address/chain changes
	...
	<div className="flex justify-between items-center mb-2">
      ...
      <div className="space-y-2">
        {paginated.length === 0 ? (
          ...
          paginated.map((tx, i) => {
            // Use the fetched name from state, with a fallback to the original tx.asset
            const displayName = nftNames[i] || tx.asset;
            return (
              ...
              <div className="flex items-center gap-3 w-full">
                <div className="flex-shrink-0">
                  <NftImage 
                    ...
                    tokenId={tx.tokenId} 
                    cache={metadataCache}
                    chainId={chainId} // Pass chainId to NftImage
                    onNameLoaded={(name) => { // Receive the correct name
                      setNftNames(prev => ({ ...prev, [i]: name }));
                    }}
                  />
                </div>
                <div className="flex-grow min-w-0">
                  ...
                  <div className="flex justify-between items-center text-sm mt-1">
                    <div className="text-black dark:text-white font-medium truncate" title={`${displayName} #${parseInt(tx.tokenId, 16)}`}>
                      {displayName} #{parseInt(tx.tokenId, 16)}
                    </div>
