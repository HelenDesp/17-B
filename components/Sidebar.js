...
// --- MODIFICATION: Added 'activeTab' and 'setActiveTab' props ---
export default function Sidebar({ activeTab, setActiveTab }) {
  ...
  const menuItems = [
    {
      ...
      path: "dashboard", // Using a simple string identifier now
    },
    {
      title: "NFTs",
      icon: (
  ...
  return (
    <div className="h-full flex flex-col bg-white border-r-2 border-dark-200 dark:bg-dark-200 dark:border-light-200 shadow-lg">
      ...
      {/* --- MODIFICATION: Updated Navigation Logic --- */}
        ...
        <ul className="space-y-1 px-2">
          {menuItems.map((item) => (
            <li key={item.title}>
              ...
              <a
                href="#" // Prevents page reload
                onClick={(e) => {
                  e.preventDefault(); // Stop the link from navigating
                  setActiveTab(item.path); // Set the active tab state
                }}
                className={`flex items-center space-x-3 px-4 py-3 transition-colors font-medium rounded-none border ${
                  activeTab === item.path // Compare with the activeTab state
