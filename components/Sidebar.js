...
import Link from "next/link"; // <-- Re-added the Link component

// The component no longer needs 'activeTab' or 'setActiveTab' props
export default function Sidebar() {
  ...
  // --- MODIFICATION: Changed paths to valid URLs ---
  const menuItems = [
    {
      title: "Dashboard",
      icon: (
          ...
          {menuItems.map((item) => (
            <li key={item.title}>
              <Link href={item.path} legacyBehavior>
                <a className={`flex items-center space-x-3 px-4 py-3 transition-colors font-medium rounded-none border ${
                    router.pathname === item.path // Now correctly compares the router's path
