...
const catData = {
  Shapes: {
    Head: { 'Round': '()', 'Parallel': '||', 'Chevron Up': '\\/', 'Chevron Down': '/\\', 'Curly': '{}', 'Square': '[]' },
    Snout: { 'Round': '()', 'Parallel': '||', 'Chevron Up': '\\/', 'Chevron Down': '/\\', 'Curly': '{}', 'Square': '[]' },
    Body: { 'Round': '()', 'Parallel': '||', 'Chevron Up': '\\/', 'Chevron Down': '/\\', 'Curly': '{}', 'Square': '[]' },
  },
  Traits: {
  ...
  return (
    <div className="relative w-full">
      <button onClick={onToggle} className="w-full flex items-center justify-between p-2 border-2 border-black dark:border-white bg-white dark:bg-gray-700 text-black dark:text-white rounded-md text-left" style={{ fontFamily: "'Cygnito Mono', monospace" }}>
        <div className="flex items-center"><span className="font-bold">{label}</span>{selected !== 'None' && (<><span className="mx-2 text-gray-400">•</span><span className="font-normal">{selected}</span></>)}</div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-700 border-2 border-black dark:border-white rounded-md z-10 max-h-48 overflow-y-auto">
          {displayOptions.map(optionName => (<button key={optionName} onClick={() => handleSelect(optionName)} className="w-full text-left p-2 flex items-center border-b border-transparent hover:border-black dark:hover:border-white" style={{ fontFamily: "'Cygnito Mono', monospace" }}><span className="mr-2 text-lg">•</span><span>{optionName}</span></button>))}
        </div>
      )}
    </div>
  );
...
export default function Petz({ ownerNFTImage }) {
  ...
  const asciiArt = useMemo(() => {
    ...
    // Combine parts
    const line1 = ears || headwear;
    const line2 = `${hShape.slice(0, 1)}${face}${hShape.slice(-1)}`;
    const line3 = `${sShape.slice(0, 1)}${snoutTrait}${sShape.slice(-1)}`;
    const line4 = `${bShape.slice(0, 1)}${outfit}${bShape.slice(-1)}`;
    const line5 = feet;
    ...
    <div className="flex flex-col items-center bg-gray-200 dark:bg-gray-900 rounded-md border border-black dark:border-white">
      ...
      <div className="w-full h-64 relative bg-blue-200 dark:bg-blue-900/50 rounded-t-md overflow-hidden flex items-center justify-center">
        ...
        <div className="z-10 p-4 rounded-lg">
          <pre className="font-mono text-3xl leading-tight text-center text-black dark:text-white" style={{ fontFamily: '"Doto", monospace', fontWeight: 900, textShadow: '1px 0 #000, -1px 0 #000, 0 1px #000, 0 -1px #000, 1px 1px #000, -1px -1px #000, 1px -1px #000, -1px 1px #000' }}>
              {asciiArt}
          </pre>
        </div>
      </div>
      <div ref={controlsRef} className="w-full p-4 bg-gray-300 dark:bg-gray-800 rounded-b-md border-t border-black dark:border-white">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Shape Selectors */}
          <div>
            <h4 className="font-bold text-lg mb-2 text-center text-gray-800 dark:text-gray-200">Shape</h4>
            <div className="space-y-2">
              <TraitSelector label="Head Shape" options={catData.Shapes.Head} selected={headShape} onChange={setHeadShape} isOpen={openDropdown === 'Head Shape'} onToggle={() => toggleDropdown('Head Shape')} />
              <TraitSelector label="Snout Shape" options={catData.Shapes.Snout} selected={snoutShape} onChange={setSnoutShape} isOpen={openDropdown === 'Snout Shape'} onToggle={() => toggleDropdown('Snout Shape')} />
              <TraitSelector label="Body Shape" options={catData.Shapes.Body} selected={bodyShape} onChange={setBodyShape} isOpen={openDropdown === 'Body Shape'} onToggle={() => toggleDropdown('Body Shape')} />
            </div>
          </div>
          {/* Trait Selectors */}
          <div>
            <h4 className="font-bold text-lg mb-2 text-center text-gray-800 dark:text-gray-200">Traits</h4>
            <div className="space-y-2">
