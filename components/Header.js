...
export default function Header({ toggleSidebar }) {
  ...
  // 2. Corrected useEffect for BNS (.base) using a direct contract read.
  useEffect(() => {
    setBaseName(null);
    ...
    const resolveBaseName = async () => {
        try {
            // This is the direct lookup on the BNS contract.
            const name = await publicClient.readContract({
                address: '0x4951A22155D5514216445513475685CF45715233', // BNS (Star Protocol) Contract on Base
                abi: [
                    {
                ...
                functionName: 'getDomainByAddress',
                args: [address]
            });

            // If the function returns a non-empty string, a name was found.
            if (name) {
                setBaseName(name);
            }
        } catch (error) {
            // The contract may revert if no name is set. We catch this error gracefully.
            console.log("Could not resolve BNS name (this is expected if none is set).", error.message);
            setBaseName(null);
        }
    };
  ...
  // Chain-aware display logic (this remains the same and is correct)
  ...
  return (
    <header className="relative sticky top-0 z-50 bg-white overflow-x-hidden w-full max-w-full shadow-md dark:bg-dark-200 dark:shadow-white/38 transition-colors duration-200">
		...
		{/* Just ensure the button displays the `displayName` variable. */}
        ...
        <div className="header-wallet-info">
          <button onClick={toggleTheme} className="flex items-center gap-2 xsm:px-1 px-3 py-2 text-dark-200 dark:text-light-200 transition-colors" aria-label="Toggle dark mode">
            {isHomePage && (
              <span className="text-sm uppercase text-dark-200 [font-family:'Cygnito_Mono',sans-serif] dark:text-light-200 [@media(max-width:540px)]:hidden">
                THEME
              </span>
            )}		
            {theme === "dark" ? (
