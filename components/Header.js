...
export default function Header({ toggleSidebar }) {
  ...
  // --- STATE AND CLIENTS ---
  const [baseName, setBaseName] = useState(null); // State to hold the resolved BNS name
  const publicClient = usePublicClient({ chainId: base.id }); // A client specifically for the Base network

  // --- HOOKS FOR NAME RESOLUTION ---
  // 1. Hook for ENS (.eth) on Ethereum Mainnet. This will only be used when not on Base.
  const { data: ensName } = useEnsName({ address, chainId: 1 });

  // 2. Robust useEffect for BNS (.base) on the Base network.
  useEffect(() => {
    // Always reset the name when the address or chain changes
    setBaseName(null);

    // Only proceed if we are on Base and have the necessary client and address
    ...
    const resolveBaseName = async () => {
        try {
            // Use viem's built-in getEnsName, which correctly handles the full resolution flow
            const name = await publicClient.getEnsName({ address });
            if (name) {
                setBaseName(name);
            }
        } catch (error) {
            console.error("Could not resolve BNS name:", error);
        }
    };

    resolveBaseName();
  }, [address, chain, publicClient]); // Rerun when address, chain, or client changes
  ...
  // --- DETERMINE DISPLAY NAME WITH CHAIN-AWARE LOGIC ---
  let displayName = formatAddress(address);
  if (chain?.id === base.id && baseName) {
    // If on Base and BNS name is found, use it.
    displayName = baseName;
  } else if (chain?.id !== base.id && ensName) {
    // If NOT on Base and ENS name is found, use it.
    displayName = ensName;
  }
