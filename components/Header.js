...
// No longer need usePublicClient, base, or namehash for this!
...
export default function Header({ toggleSidebar }) {
  ...
  // --- 1. GET ENS NAME (ETHEREUM MAINNET) ---
  // This hook looks for a .eth name on chainId 1.
  const { data: ensName } = useEnsName({ address, chainId: 1 });

  // --- 2. GET BNS NAME (BASE) ---
  // This hook specifically looks for a .base name on chainId 8453 (Base).
  // It's enabled only when the user is connected to the Base network.
  ...
  // The manual `resolveBaseName` useEffect has been removed.
  // The useEnsName hooks above are the correct way to handle this.
  ...
  useEffect(() => {
    const fetchEthPrice = async () => {
      ...
      const nativeIdMap = {
        1: 'ethereum',
        8453: 'ethereum', // Base uses ETH as its native token
        42161: 'ethereum',
        10: 'ethereum',
        137: 'matic-network',
        56: 'binancecoin',
      ...
      <div className="header-container flex items-center justify-between px-2 w-full max-w-[100vw] overflow-hidden">
        ...
        <div className="header-wallet-info">
          ...
          {isConnected ? (
            ...
            >
              {/* --- 3. DISPLAY THE CORRECT NAME --- */}
              {/* This logic now correctly prioritizes the BNS name, then the ENS name, and finally the address. */}
