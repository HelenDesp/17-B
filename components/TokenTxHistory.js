            ...
            <div key={i} className="text-sm text-gray-200 dark:text-gray-100 border-b border-gray-200 dark:border-gray-100 pb-2">
              ...
              {expandedIndexes[i] && (
                <div className="mt-2 space-y-1 text-xs">
                  <div className="text-black dark:text-white"><strong>From:</strong> <a href={`${getExplorerBaseUrl(chainId)}/address/${tx.from}`} target="_blank" rel="noopener noreferrer" className="underline text-black dark:text-white">{shortenAddress(tx.from)}</a></div>
                  <div className="text-black dark:text-white"><strong>To:</strong> <a href={`${getExplorerBaseUrl(chainId)}/address/${tx.to}`} target="_blank" rel="noopener noreferrer" className="underline text-black dark:text-white">{shortenAddress(tx.to)}</a></div>
					<div className="text-black dark:text-white">
