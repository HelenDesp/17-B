...
// --- NEW, OPTIMIZED AddressDisplay COMPONENT ---
// This component contains all the logic for optimized name resolution.
function AddressDisplay({ address, chainId, getExplorerBaseUrl, shortenAddress }) {
  ...
  // BNS Hook: Only enabled if the current chain is Base.
  const { data: bnsName } = useName({
    address: address,
    chain: base,
    enabled: !!address && chainId === base.id,
  });

  // ENS Hook: Only enabled if the current chain is Ethereum or Sepolia.
  const { data: ensName } = useEnsName({
    address: address,
    chainId: 1,
    enabled: !!address && (chainId === mainnet.id || chainId === sepolia.id),
  });

  useEffect(() => {
    let finalResolvedName = null;
    
    // Logic to select the correct name based on the current chain.
    switch (chainId) {
      case base.id:
      ...
      case sepolia.id:
        ...
        break;
      default:
        // For all other chains, do nothing.
        break;
    }

    setDisplayName(finalResolvedName || shortenAddress(address));
    
  }, [address, chainId, bnsName, ensName, shortenAddress]);
...
export default function TokenTxHistory({ address, chainId, isConnected }) {
  ...
  // --- THE `useRef` GUARD TO PREVENT MULTIPLE FETCHES ---
  const fetchController = useRef({ lastFetchedKey: null });
  
  const getChainLabel = (cId) => {
    switch (cId) { case 1: return "Ethereum"; case 8453: return "Base"; case 137: return "Polygon"; case 42161: return "Arbitrum"; case 10: return "Optimism"; case 11155111: return "Sepolia"; case 56: return "BNB"; default: return "Base"; }
  };  

  const getExplorerBaseUrl = (cId) => {
    ...
    const fetchTxs = async () => {
      try {
        const [sentRes, receivedRes] = await Promise.all([
          fetch(ALCHEMY_BASE_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "alchemy_getAssetTransfers", params: [{ fromBlock: "0x0", fromAddress: address, category: ["external", "erc20"], withMetadata: true, excludeZeroValue: true, maxCount: "0x32" }] }) }),
          fetch(ALCHEMY_BASE_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 2, method: "alchemy_getAssetTransfers", params: [{ fromBlock: "0x0", toAddress: address, category: ["external", "erc20"], withMetadata: true, excludeZeroValue: true, maxCount: "0x32" }] }) })
        ]);
